...
@btime begin
  ...
  adf, mdf = run!(sheepwolfgrass, sheepwolf_step!, grass_step!, 100; adata, mdata)
end;
...
@present TheoryWS <: SchGraph begin
  ...
  (Eng, Time)::AttrType
  countdown::Attr(V, Time); sheep_eng::Attr(Sheep, Eng); wolf_eng::Attr(Wolf, Eng)
end

@acset_type WS_Generic(TheoryWS, part_type=BitSetParts) <: HasGraph
const WS = WS_Generic{Int, Int}

# Swap wolves and sheep
F = Migrate(
...
function create_grid(n::Int)
  ...
  for i in 0:n-1  # Initialize grass 50% green, 50% uniformly between 0-30
    for j in 0:n-1
      coords[i=>j] = add_part!(lv, :V; countdown=max(0, rand(-30:30)))
    end
  end
  for i in 0:n-1
    for j in 0:n-1
...
se_rule = Rule(S; expr=(Eng=[vs -> vs[1] + 4], Time=[vs -> 30],),
...
sheep_reprod_rule = Rule(
  hom(G, S),
  hom(G, s_reprod_r);
  expr=(Eng=fill(vs -> round(Int, vs[1] / 2, RoundUp), 2),)
);
...
overall = while_schedule(cycle, curr -> nparts(curr, :Wolf) >= 0)  # wrap in a while loop
...
@btime interpret!(overall, deepcopy(X); maxstep=100);
