...
n = length(ladj)
names = [Symbol("Rgn$i") for i in 1:n]

typed_nbr = pairwise_id_typed_petri(infectious_ontology, :Pop, :infect, names)

typed_nbr_aug = add_reflexives(
  typed_nbr,
  [[:disease], [:disease], [:disease]],
  infectious_ontology
)

nbr_uwd = @relation () where (Country1::Pop, Country2::Pop) begin
